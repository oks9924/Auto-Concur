...
**참석자 칸은 비워두면 창에 적어둔 사람이 들어간다.** 이 칸에는 수식을 넣지
않는다 — 수식이 들어 있으면 이름을 덧붙이기 어렵다. 늘 혼자면 그냥 비워두고,
다른 사람이거나 여러 명이면 **쉼표로 적으면 된다**(`kyungsik.oh, hong.gildong`).
적힌 것이 있으면 그것이 우선이다.

경비유형이 식음료가 아닌 행은 비워둬도 참석자를 넣지 않는다.
