...
**이미 입력된 명세는 건드리지 않는다.** 명세가 만들어져 있으면 칸이 잠겨서
고칠 수 없다. 그때는 그냥 지나간다 — 사람이 넣어둔 값을 우리가 다시 쓸 이유가
없다. 표가 아예 없는 경우, 칸이 전부 잠긴 경우, 칸이 전부 채워져 있는 경우
셋 다 지나간다. 그러면 명세 탭에는 저장할 것이 없으므로 상세 정보 탭으로
돌아가 거기서 넣은 값만 저장한다.
...
이미 끝났으니 넣은 값은 남아 있다. 그래서 저장할 때마다 주소로 상세를 다시 열고
금액으로 맞는 경비인지 확인한 뒤 다음 단계로 간다. 탭 클릭에 기대지 않는다.
...
| 참석자 | 화면과 견줘서 빠진 사람만 넣는다 |

참석자는 이름 형태가 달라도 같은 사람으로 본다 — 작업지의 `kyungsik.oh` 와 화면의
`Oh Kyungsik` 은 같다고 판단한다(검색어를 토막 내서 전부 이름에 들어 있으면 같은
사람으로 본다).

화면에서는 **행 전체 글자**를 읽어서 견준다. 어느 칸이 이름인지 짐작하지 않는다 —
첫 칸을 이름으로 읽었더니 체크박스 칸의 `행 선택`이 나와서, 이미 맞게 들어 있는
참석자를 전부 '다르다'고 판단한 적이 있다.

**참석자는 기본적으로 지우지 않는다.** 화면을 잘못 읽어 멀쩡한 참석자를 지운 적이
있다(2026-08). 잘못 지우면 되돌릴 방법이 없다. 작업지에 없는 참석자가 보이면
아무것도 바꾸지 않고 멈추면서 `inspect-out/attendee-rows.json` 에 화면을 남긴다.

지우려면 `--remove-attendees` 를 붙여야 한다. 그때는 행 끝의 작업(`...`) 버튼을
눌러 메뉴에서 제거를 고르고, 누른 뒤 정말 사라졌는지 확인한다.

**넣지 못했으면 저장하지 않는다.** 저장 직전에 목록을 다시 읽어서 넣으려던 사람이
실제로 올라와 있는지 본다. 없는데 저장하면 남아 있던 참석자까지 지워진 채로
굳는다 — 실제로 그렇게 됐었다. 넣다가 실패했을 때도 저장하지 않고 모달을 닫는다.

읽는 범위가 중요하다. **모달 안만 본다.** 화면 전체에서 표 행을 훑었더니 모달
