...
**참석자 칸에는 창에 적어둔 사람이 값으로 들어간다.** 수식이 아니라 값이다 —
수식이면 이름을 하나 더 붙이려 할 때 통째로 지우고 다시 쳐야 한다. 값이니까
뒤에 `, hong.gildong` 만 붙이면 된다.

모든 행에 들어가지만 **실제로 넣는 것은 식음료 행뿐이다.** 참석자 칸은 그 유형에만
있어서, 주차비 같은 행에 넣으려 하면 그 버튼이 없어 실패한다. 그래서 반영할 때
유형을 보고 거른다.
