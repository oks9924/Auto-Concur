...
**참석자는 작업지와 맞춘다.** 없는 사람은 지우고, 빠진 사람은 넣는다. 바꿀 것이
없으면 모달을 닫고 지나간다.

지우는 방법: 그 행의 체크박스를 고르고 툴바의 `제거`를 누른다. 고르기 전에는
`제거`가 비활성이라 눌리게 된 다음에 누른다. 누른 뒤 정말 사라졌는지 확인한다.

화면 셀렉터 (2026-08 확인):

| 요소 | 셀렉터 |
|---|---|
| 참석자 모달 | `[data-nuiexp="attendees-dialog"]` |
| 참석자 행 | `[role="row"][data-testid="data-row"]` (모달 안) |
| 이름 | 행 안의 `[data-nuiexp="name"]` |
| 행 선택 | 행 안의 `[data-testid="selection-cell"] input[type=checkbox]` |
| 제거 | `[data-nuiexp="sat-btn-remove-attendee"]` |
| 저장 / 취소 | `[data-nuiexp="sat-btn-save"]` / `sat-btn-cancel` |

**표가 `<table>`이 아니라 `div[role="table"]`이다.** `tbody tr` 로 찾다가 한 줄도
못 읽었고, 그래서 이미 맞게 들어 있는 참석자를 전부 '다르다'고 판단했다. 그 상태로
지우기까지 해서 멀쩡한 참석자가 사라진 적이 있다(2026-08). 지금은 훅으로 읽는다.

**있다는데 한 줄도 못 읽으면 아무것도 바꾸지 않는다.** 참석자 수는 N명인데 목록이
비어 보이면 화면을 잘못 읽고 있는 것이다. 그 상태의 판단으로 지우면 안 된다.
