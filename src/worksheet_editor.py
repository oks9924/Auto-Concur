...
class Editor(tk.Toplevel):
    ...
    def run(self):
        ...
        if self.mileage_panel is not None:
            rows = self.mileage_panel.book.rows
            pending = sum(r.get('concur_state') not in ('verified', 'needs_review') for r in rows)
            verified = sum(r.get('concur_state') == 'verified' for r in rows)
            review = sum(r.get('concur_state') == 'needs_review' for r in rows)
            text += (f'\n\n마일리지 전체 {len(rows)}건 · 이번 신규 생성 대상 {pending}건'
                     f' · 기존 생성 확인 {verified}건 · 확인 필요 {review}건'
                     '\n저장 후 카드 경비 처리 다음에 같은 Concur 세션에서 이어서 생성합니다.'
                     '\n기존 생성 확인/확인 필요 행은 자동으로 다시 생성하지 않습니다.')
